...
import com.example.ft.ChatActivity.ImageUtils
...
class Admin_Chat_Activity : AppCompatActivity() {
    ...
    private fun sendMessage(type: String, refNo: String, text: String) {
        ...
        ref.update(mapOf(
            ...
            "unreadCountClient" to FieldValue.increment(1))
        )
    }
    ...
    private fun sendFileMessage(fileType: String, url: String) {
        ...
        val ref = db.collection("Jobs").document(jobType).collection("Reference Number")
            .document(intent.getStringExtra("refNo")!!)
